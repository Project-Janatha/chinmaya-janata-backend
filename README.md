...
Backend code for Chinmaya Janata

Our backend runs on AWS, using DynamoDB and ECS
...
We are using DynamoDB to store user, center and event data. User persistence is managed by JSON Web Tokens (JWT) on the frontend

## Backend

The backend is managed using Elastic Container Services (ECS) which Dockerizes our ExpressJS backend